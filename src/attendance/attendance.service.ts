...
export class AttendanceService {
  ...
  async create(createAttendanceDto: CreateAttendanceDto): Promise<Attendance | null> {
    try {
      const { classId, teacherId, attendanceDate, studentsAttendance, ...rest } = createAttendanceDto;
  ...
  async findAll(classId: string): Promise<any[]> {
    try {
  ...
  async findOne(studentId: string, schoolId: Types.ObjectId, month?: number): Promise<any> {
    try {
  ...
  async updateByClass(classId: string, updateAttendanceDto: UpdateAttendanceDto, schoolId: Types.ObjectId): Promise<Attendance | null> {
    try {
      ...
      if (!classroom) {
        throw new NotFoundException('Classroom not found or does not belong to the specified school');
      }
  ...
  async updateByStudent(
    ...
    try {
              ...
              { $match: {
                  $expr: {
                    $and: [
                      { $eq: ['$classId', '$$classId'] },
                      { $in: ['$$studentId', '$studentsAttendance.studentId'] },
                      { $in: ['$attendanceDate', attendanceUpdates.map(update => update.date)] }
                    ]
                  }
              }},
              { $addFields: {
                      ...
                      in: {
                        $cond: [
                          ...
                          { $mergeObjects: ['$$sa', { 
                            status: {
                              $arrayElemAt: [
                                attendanceUpdates.map(update => update.status),
                                { $indexOfArray: [attendanceUpdates.map(update => update.date), '$attendanceDate'] }
                              ]
                            }
                          }] },
                          '$$sa'
      ...
      if (result.length === 0) {
        throw new NotFoundException('Student not found or attendance records not updated');
      }
